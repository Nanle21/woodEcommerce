...
{
	return "Linking Construction Ltd";
}
...
function echo_msg($the_msg,$the_error_type="error_msg")
{
	if($the_error_type=="success_msg")
	{
		$class_to_use="alert alert-success";
		$sign_to_use="glyphicon glyphicon-ok-circle";
	}
	elseif($the_error_type=="error_msg" || $the_error_type!="success_msg")
	{
		$class_to_use="alert alert-danger";
		$sign_to_use="glyphicon glyphicon-remove-circle";
	}
	
     $the_content="<div class='container-fluid'>
	 					<div class='msg'>
						<div class='". $class_to_use ." alert-dismissible' role='alert'>
							<div class='row'>
								<div class='col-lg-1 text-left'>
									<b><span class='". $sign_to_use ."' style='font-size:24px; padding-right:10px;'></span></b>
								</div>
								<div class=' col-lg-10' style='padding-left:0px;'>". $the_msg ."</div>
...
{
	$acc_id="V000000";
	$get_acc_id=table_open("orders","","order_num","DESC",__FILE__,__LINE__);
	if($get_acc_id['error']=="")
	{
		$acc_id=$get_acc_id[0]['order_num'];
	}
	$new_acc_id=strval(intval(substr($acc_id,1,6))+1);
	while(strlen($new_acc_id)<6)
	{
...
{
	$hire_id="I00000";
	$get_last_hire_id=table_open("hire","","hire_id","DESC",__FILE__,__LINE__);
	if($get_last_hire_id['error']=="")
	{
		$hire_id=$get_last_hire_id[0]['hire_id'];
	}
...
{
	$sale_id="S00000";
	$get_last_sale_id=table_open("sales","","sale_id","DESC",__FILE__,__LINE__);
	if($get_last_sale_id['error']=="")
	{
		$sale_id=$get_last_sale_id[0]['sale_id'];
	}
...
function get_customer_id()
{
	...
	$get_last_contractor_id=table_open("contractor","","contractor_id","DESC",__FILE__,__LINE__);
	if($get_last_contractor_id['error']=="")
	{
		$contractor_id=$get_last_contractor_id[0]['contractor_id'];
	}
...
function get_contractor_cat_ID()
{
	$catcon_id="X00000";
	$get_last_cat_con=table_open("contract_cat","","catcon_id","DESC",__FILE__,__LINE__);
	if($get_last_cat_con['error']=="")
	{
		$catcon_id=$get_last_cat_con[0]['catcon_id'];
	}
...
function get_MLM_company_info($id,$info_to_get)
{
	$result="";
	
	$company=table_open("mlm_companies","id='$id'","","",__FILE__,__LINE__);
	
	if($company['error']=="")
	{
		$result=$company[0][$info_to_get];
	}
